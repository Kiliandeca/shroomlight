...
export class Client {
  ...
  listenPositionUpdate() {
    this.socket.on('look', (look) =>
      this.playerEntity.updateLocation({
        pitch: angleToDegree(look.pitch),
        yaw: angleToDegree(look.yaw),
      })
    );
    ...
    this.socket.on('position_look', (positionLook) => {
      ...
      this.playerEntity.updateLocation({
        position,
        pitch: angleToDegree(positionLook.pitch),
        yaw: angleToDegree(positionLook.yaw),
      });
    });
  }
