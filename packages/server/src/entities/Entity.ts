...
export class Entity {
  ...
  pulsePhysic() {
    ...
    const pendingBlock = this.world.getPlaceHolderChunk().getBlock(pendingPosition)
    if (pendingBlock.type !== 0) {
      const pendingPositionY = this.location.position.clone().add(new Vec3(0, this.velocity.y, 0))
      const pendingBlockY = this.world.getPlaceHolderChunk().getBlock(pendingPositionY)
      if (pendingBlockY.type !== 0) {
        this.velocity.y = Math.ceil(pendingPositionY.y) - this.location.position.y
      }
    }
